  ...
  const [{ data: messages }, { data: modelResponses }] = await Promise.all([
    ...
    conv.mode === "compare"
      ? sc
          ...
          .select(
            "id, provider, model, content, tokens_in, tokens_out, cost_usd, latency_ms, error, score_accuracy, score_clarity, score_creativity, score_usefulness, score_risk, position, created_at"
          )
          .eq("conversation_id", conv.id)
